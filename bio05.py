...
def find_reverse_complement(pattern):
    ...
    for i in range(len(pattern)):
        r_pattern += reverse_part(pattern[len_p - i - 1])
    return r_pattern


def freq_w_mismatch_reverse(text, k, d):
    patterns = []
    for i in range(4 ** k):
    ...
    cur_max_pats = []
    for pat in patterns:
        num = find_approx_pattern_count(text, pat, d) + find_approx_pattern_count(text, find_reverse_complement(pat), d)
        if (num > cur_max):
            cur_max_pats = []
            cur_max = num
...
def final_result_hw5(text, k, d):
    res = freq_w_mismatch_reverse(text, k, d)
    st = ""
